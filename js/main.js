...
text1.innerHTML='Chào cậuuuu';
text1.classList.add('text-1')

text2.innerHTML='Cậu nói là cậu không thích hoa, nên...';
text2.classList.add('text-2')

text3.innerHTML='Tớ tặng cậu cái này nhé!! ^_^';
text3.classList.add('text-3')

text4.innerHTML='Với cả...';
text4.classList.add('text-4')
...
myPromise.then(
  ...
  function(value) {
    setTimeout(() => {
      typewriter.removeChild(text1);
      typewriter.appendChild(text2);
    }, 3500);
  },
  function(error) { /* code if some error */ }
)
.then(
  function(value) {
    setTimeout(() => {
      typewriter.removeChild(text2);
      typewriter.appendChild(text3);
    }, 7500);
  },
  function(error) { /* code if some error */ }
)
.then(
  function(value) {
    setTimeout(() => {
      typewriter.removeChild(text3);
      typewriter.appendChild(text4);
    }, 11500);
  },
  function(error) { /* code if some error */ }
).then(
  function(value) {
    setTimeout(() => {
      typewriter.removeChild(text4);
      $("body").append(c1.canvas);
      container2.classList.remove('hide')
      title.classList.remove('hide')
      const pinkboard = document.querySelector('#pinkboard')
      pinkboard.width=1023
      pinkboard.height=429
    }, 14000);
  },
  function(error) { /* code if some error */ }
  );
